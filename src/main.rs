...
fn local_addon_resource_payload(
    resource: &str,
    media_type: &str,
    id: &str,
    extra: Option<&str>,
...
async fn opensub_hash() -> Json<Value> {
    Json(json!({
        "error": null,
        "result": null
    }))
...
async fn subtitles_tracks() -> Json<Value> {
    Json(json!({
        "error": null,
        "result": { "tracks": [] }
    }))
...
async fn subtitles_proxy(
    ...
    match fetch_subtitle(&state.client, &url).await {
        Ok(response) => {
            let upstream_disposition = response
                .headers()
            ...
            match response.bytes().await {
                Ok(bytes) if !bytes.is_empty() => {
                    let (content_type, body) = subtitle_response_body(&ext, &bytes);
...
fn subtitle_response_body(ext: &str, bytes: &Bytes) -> (&'static str, Bytes) {
    match ext.to_ascii_lowercase().as_str() {
        "vtt" => {
            let text = String::from_utf8_lossy(bytes);
            if text.trim_start().starts_with("WEBVTT") {
                ("text/vtt; charset=utf-8", bytes.clone())
            } else {
                ("text/vtt; charset=utf-8", Bytes::from(srt_to_vtt(&text)))
            }
        }
        "srt" => ("application/x-subrip; charset=utf-8", bytes.clone()),
